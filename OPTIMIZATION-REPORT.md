...
> ⚠️ 但上面这组数据**当时完全没有看画面**。下面的第二轮补上了这一课，并且立刻抓出了一个 A/B 表看不出来的回归。

---

## 三·补、第二轮：真正把画面打开看之后

第一轮的验证全在计数器上（draw call、几何体数、FPS、DOM 断言）。计数器不会告诉你粒子是方的、也不会告诉你关卡地形变了。补上截图后，找到 3 个新问题。

### 🐞 Bug 5：`Island.ts` 改动顺带改了种子随机流，**同一个种子生成出了不同的关卡**

几何合并时，碎片循环里的 `rng()` 调用次数和顺序被改了：

```ts
// 改前：尺寸在循环外抽一次，每片碎片的顺序是 a → rr → y → rotX/rotY/rotZ（6 次）
const shardGeo = new THREE.TetrahedronGeometry(0.12 + rng() * 0.1, 0);
const count = 2 + Math.floor(rng() * 3);
for (...) { a; rr; y; s.rotation.set(rng(), rng(), rng()); }

// 改后：尺寸挪进循环（每片多抽 1 次），y 挪到最后（7 次）
const count = 2 + Math.floor(rng() * 3);
for (...) { size; a; rr; TMP_EULER.set(rng(), rng(), rng()); y; }
```

`random.ts` 存在的前提就是 `seed(42)` 可复现。多抽一次就会让**之后所有随机决策整体错位** —— 草叶密度和高度、裂缝旋转角全部改变。截图里肉眼可见：同一场景的草叶从稀疏变密、变高。

**发现方式**：冻结同一帧（`seed(42)` + `setState('active-play')` + `setPausedForScreenshot(true)`），改动前后各截一张做像素差：

| 对比 | mean | 差异 > 8/255 | 差异 > 32/255 |
|---|---:|---:|---:|
| 改动前 vs 改动后（修复前） | 4.015 | **11.14%** | **3.43%** |
| 改动前 vs 改动后（修复后） | 0.855 | **2.58%** | **0.99%** |
| 噪声底线（同版本跑两次） | 0.965 | 2.79% | 1.21% |

修复后差异**低于噪声底线**，即两个版本已渲染等价。差异图里只剩草叶抖动，没有任何整块几何缺失。

**修复**：把尺寸抽回循环外，并按原顺序显式取 `yRng / rx / ry / rz`。同时让碎片 `clone()` 自一个共享基础几何体 —— 既还原了原代码「所有碎片共用一个 `TetrahedronGeometry`」的行为，也保证 `mergeGeometries()` 不会因属性不一致而静默返回 null（那样会只剩第一层岩石）。已加注释防止以后再被"顺手整理"。

### 🐞 Bug 6：粒子渲染成**硬边方块**，而且根本不会淡出

`PointsMaterial` 没有设置 `map`。three.js 会把每个点画成一个屏幕对齐的实心四边形 —— 所以火花效果在画面里是一堆**白色小纸片**，散落在岛上和空中，非常显眼。

另外两个连带问题：

- 逐点 `size` 属性是**死代码**。`PointsMaterial` 只有全局统一尺寸，完全忽略 `size` 顶点属性，所以 `sizes[i]`（0.45 / 0.7 / 1.0）从来没有生效过。
- 淡出写的是 `colors[i] *= 0.999`（每帧）。在 0.4 秒寿命内累计只有约 3%，等于**没有淡出**，粒子是"啪"地直接消失。

**修复**：加一张惰性生成的 64×64 径向衰减贴图（方块 → 柔和圆形光点）；删掉无效的 size 属性；改为每帧用 `base * (life / maxLife)` 重算颜色 —— 在加色混合下线性变黑就是真正的淡出。

保留 `PointsMaterial` 是刻意的：它自带尺寸衰减和场景 `FogExp2` 雾效，换成手写 `ShaderMaterial` 就得自己重实现这两样。

### 🔴 发现 7：部署资源 **19.58 MB** 的 PNG（这是第一轮完全漏掉的最大一项）

第一轮只盯着 GPU 开销，没量过下载体积。实际：

| 资源 | 体积 | 显示尺寸 | 问题 |
|---|---:|---|---|
| `previews/stage-*.png` ×5 | **6.97 MB** | 88×66 CSS px | 1024×1024，面积上浪费约 **136 倍** |
| `textures/*.png` ×9 | **12.61 MB** | 3D 贴图 | 全是 PNG，且都带无用的 alpha |

而且**每一个文件的 alpha 通道都是纯不透明的**（`alpha_min == alpha_max == 255`）—— PNG 白白存下来，还额外增加约 1/4 体积。

这个问题在截图里是**看得见的**：选关页每张卡片左侧的缩略图是**空白的** —— 因为 7 MB 在 500ms 内根本没加载完。

**修复**（`tools/optimize-assets.py`，可重复执行）：

| 资源 | 改动前 | 改动后 | 降幅 |
|---|---:|---:|---:|
| 预览图（居中裁成 4:3，264×198） | 6.97 MB | **59 KB** | **99.2%** |
| 贴图（尺寸不变，去 alpha，quality 88） | 12.61 MB | **1.32 MB** | **89.5%** |
| **合计** | **19.58 MB** | **1.35 MB** | **93.1%** |

原始 PNG 移到 `raw-assets/`（在 `public/` 之外，Vite 既不服务也不拷贝），美术源文件仍在工作区，只是不再进入部署产物。

**验证**：冻结同帧对比 WebP 与 PNG 渲染 —— 差异 2.836%（>8/255），噪声底线 2.794%，**处于噪声之内**；差异图无整块区域，说明没有贴图静默加载失败。

---

## 四、还可以继续优化的地方（按收益排序）

### 🔴 0.（已完成）资源体积 19.58 MB → 1.35 MB

见上方「发现 7」。这是全部条目里投入产出比最高的一项，第一轮漏掉了。
...
### 🟠 8. 漏捡钥匙会把玩家**软锁在终点门前**（看画面才发现的玩法问题）

修复 Bug 1 之后，玩家终于能站到终点门前面了（截图确认：进度 100%、门就在眼前、`mode` 仍是 `playing`）。但门是锁的：

```ts
if (this.keysRequired > 0 && !this.bossUnlocked) {
  ...
  continue;   // 缺钥匙时直接跳过通关判定
}
```

`keysRequired` 颗钥匙必须**全部**捡到才能通关。而这是个自动向前跑的游戏，身后的浮岛已经过去、回不去，玩家在门前没有任何出路 —— 只能按 Esc 手动「重新开始」。每 1.6 秒弹一次「还需 N 颗钥匙」提示了原因，但没有给出路。

**建议**（属于玩法决策，我没有擅自改）：
- 门附近给一次性的明确提示，例如「按 Esc 可重新开始本关」；
- 或者允许缺钥匙时以惩罚方式通过（扣水晶 / 降星级）；
- 或者给一个「回到起点」的传送。

### 🟡 9. 弹跳岛 / 移动岛在 bloom 下过曝成"气球"

`bouncy` 岛的岛盖是压扁到 0.55 的半球（`SphereGeometry` + `scale(1, 0.55, 1)`），配色 `#5ec4a8` 本身没问题，但它同时带 `emissive: '#0d4a3c'`（强度 0.35），再叠 `UnrealBloomPass`，最终在屏幕上接近纯白，看起来像一个半透明的巨大气球，挡住大片视野。`moving` 岛（`emissive: '#1a3a60'`）同理。

**建议**：把这两类岛的自发光强度从 0.35 降到 0.15 左右，或把 bloom 的 `threshold` 调高，让自发光不再触发泛光。这是纯调色，不影响逻辑。
